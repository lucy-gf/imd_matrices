...
cont_imd_distr <- readRDS(file.path("output", "data", "cont_matrs",sens_analysis_folder,"cont_imd_distr.rds")) %>% 
  mutate(c_location = tolower(c_location))
poly_weights <- readRDS(file.path("data", "ons","polymod_weights.rds")) %>% 
  mutate(c_location = tolower(c_location))
reconnect_weights <- readRDS(file.path("output", "data","cont_matrs","reconnect_weights.rds")) %>% 
  mutate(c_location = tolower(c_location))
