...
## depends if in regional sensitivity analysis:
...
if(sens_analysis != 'regional'){
  ...
  if(sens_analysis == 'regional'){
    cm1000 <- cm1000[order(bootstrap_index, p_engreg, p_imd_q, p_age_group, c_imd_q, c_age_group)]
  }else{
  ...
  ###.##############
    ...
    for(reg in unique(cm1000$p_engreg)){
      ...
      demog <- demog_allreg %>% filter(p_engreg == reg)
      cm1000_specreg <- cm1000[p_engreg == reg]
      cm1000_specreg[, p_engreg := NULL]
      ...
      ## run model x1000
      n_bs <- max(cm1000_specreg$bootstrap_index)
      ...
      for(sim_num in 1:n_bs){
        ...
        cm <- cm1000_specreg[bootstrap_index == sim_num,] 
        cm$p_age_group <- factor(cm$p_age_group, levels = pars$ages); cm$c_age_group <- factor(cm$c_age_group, levels = pars$ages)
        cm <- cm[order(bootstrap_index, p_imd_q, p_age_group, c_imd_q, c_age_group)]
        cm <- cm %>% 
          mutate(p = paste0(p_imd_q, '_', p_age_group),
        ...
        if(!pset$R0fixed){betanew <- pars$beta}else{
          source(paste0(source_dir,"/R0_.r"))    
          betanew = R0(pars, cm_in = cm, R0assumed = as.numeric(pars$R0), printout = 0) 
          BETATRACK[sim_num] <- betanew
        }
        ...
        byw <- data.table(rbind(byw, cbind(mas$byw, sim = sim_num, p_engreg = reg)))
        
        byaw <- data.table(rbind(byaw, cbind(mas$byaw, sim = sim_num, p_engreg = reg)))
        
        byall <- data.table(rbind(byall, cbind(mas$byall, sim = sim_num, p_engreg = reg)))
      ...
      Iwpeakvalvec = byw[p_engreg == reg][, c('sim','Iw')][, lapply(.SD, max), by = 'sim']
      ARvec = byw[p_engreg == reg][, c('sim','Iw')][, lapply(.SD, sum), by = 'sim']
      cat('\n',paste0("Peak:  ", signif(mean(Iwpeakvalvec$Iw),digits=3)*10^(-6)," million (95% CI: ", 
                      signif(quantile(Iwpeakvalvec$Iw, 0.025),3)*10^(-6),
                      ' - ', signif(quantile(Iwpeakvalvec$Iw, 0.975),3)*10^(-6), ' million)'), sep = '')
      cat('\n',paste0("Attack rate:  ", signif(mean(ARvec$Iw)/sum(demog$Population),digits=3)*100,"% (95% CI: ", 
                      signif(quantile(ARvec$Iw/sum(demog$Population), 0.025),3)*100,
                      '% - ', signif(quantile(ARvec$Iw/sum(demog$Population), 0.975),3)*100, '%)'), sep = '')
      if(pset$R0fixed){
        cat('\n',paste0("Beta:  ", signif(mean(BETATRACK),digits=3)," (95% CI: ", 
                        signif(quantile(BETATRACK, 0.025),3),
                        ' - ', signif(quantile(BETATRACK, 0.975),3), ')\n'), sep = '')
      ...
      ## save files
      write_rds(byw, gsub('all','w',gsub('.rds', paste0('_', regional_SA, '_', gsub(' ', '_', reg), '.rds'), .args[3])))
