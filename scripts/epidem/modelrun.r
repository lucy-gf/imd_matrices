...
## RUN 1000x IF NOT IN REGIONAL SENSITIVITY ANALYSIS
...
if(sum(abs(age1 - age2) < 1) != na){stop('Age sums not aligning')}
  ...
  for(reg in unique(cm1000$p_engreg)){
    ...
    for(sim_num in 1:n_bs){
      ...
      byw <- rbind(byw, 
                   cbind(mas$byw, sim = sim_num, p_engreg = reg))
      
      byaw <- rbind(byaw, 
                    cbind(mas$byaw, sim = sim_num, p_engreg = reg))
      
      byall <- rbind(byall, 
                     cbind(mas$byall, sim = sim_num, p_engreg = reg))
